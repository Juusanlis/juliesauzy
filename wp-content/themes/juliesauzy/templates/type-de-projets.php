  <!-- Types de projets -->
  <section id="types_projets" class="types_of_projects flex justify-center items-center bg-black text-white py-14 text-center lg:min-h-[65vh]">
    <div class="container">

      <div class="title pb-10 lg:pb-20 text-center">
        <h2 data-aos="fade-zoom-in"
            data-aos-duration="1000"
            data-aos-once="true"
            >Différents types de projets proposés</h2>
      </div>

      <div class="lg:inline-flex space-y-10 lg:space-y-0 text-center justify-evenly w-full h-full">
        <div class="logo_title_descr">
          <i class="fa-solid fa-display text-5xl"></i>
          <h4 class="py-5">Sites vitrine</h4>
          <p  data-aos="fade-up"
              data-aos-delay="100"
              data-aos-duration="1000"
              data-aos-once="false"
              class="text-light-white hover:text-white duration-300 transition-all">Site web présentant une entreprise,<br> une marque, ou encore une activité,<br> sans générer de ventes directes.</p>
        </div>

        <div class="site_wp">
          <i class="fa-brands fa-wordpress-simple text-6xl"></i>
          <h4 class="py-5">Sites WordPress</h4>
          <p  data-aos="fade-up"
              data-aos-delay="250"
              data-aos-duration="1000"
              data-aos-once="false"
              class="text-light-white hover:text-white duration-300 transition-all">Développement de sites sur-mesure,<br> entièrement administrables.<br>
                                                                                    Grâce au CMS WordPress vous bénéficiez<br> d'une interface de gestion complète,<br> modifiable sur le long terme. </p>
        </div>

        <div class="site_commerce">
          <i class="fa-solid fa-shop text-5xl"></i>
          <h4 class="py-5">Sites commerce</h4>
          <p  data-aos="fade-up"
              data-aos-delay="400"
              data-aos-duration="1000"
              data-aos-once="false"
              class="text-light-white hover:text-white duration-300 transition-all">Commerce en ligne qui permet de toucher<br> une large clientèle en toute heure, et <br>contribue au développement de l'entreprise.</p>
        </div>
      </div>

    </div>

  </section>
